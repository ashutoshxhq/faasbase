...
use crate::extras::TokenResponse;
...
impl UserService {
    ...
    pub async fn _get_user_by_user_ids(&self, user_ids: Vec<Uuid>) -> Result<Vec<User>, Error> {
        let mut conn = self.pool.clone().get()?;
    ...
    pub async fn update_user(&self, user_id: Uuid, data: UpdateUser) -> Result<(), Error> {
        ...
        if let Some(firstname) = data.firstname.clone() {
            profile_update = true;
            profile_data.insert("given_name".to_string(), firstname);
        }

        if let Some(lastname) = data.lastname.clone() {
            profile_update = true;
