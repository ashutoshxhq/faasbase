...
export const Step = (props: StepProps) => {
  const { isActive, isCompleted, isLastStep, isFirstStep, title, ...stackProps } =
    props
    ...
    <Stack spacing="4" direction={{ base: 'row', md: 'column' }} flex="1" {...stackProps}>
      <Stack spacing="0" align="center" direction={{ base: 'column', md: 'row' }}>
        <Divider
          ...
          borderWidth="1px"
          borderColor={isFirstStep ? 'transparent' : isCompleted || isActive ? 'orange.500' : 'inherit'}
        />
        <StepCircle isActive={isActive} isCompleted={isCompleted} />
        <Divider
          orientation={orientation}
          borderWidth="1px"
          borderColor={isCompleted ? 'orange.500' : isLastStep ? 'transparent' : 'inherit'}
        />
      </Stack>
      <Stack
        spacing="0.5"
