...
async fn main() {
    ...
    loop {
        ...
        let rcv_message_output = client.receive_message().set_wait_time_seconds(Some(10)).max_number_of_messages(1).queue_url(queue_url.clone()).send().await.unwrap();

        for message in rcv_message_output.messages.unwrap_or_default() {
            let application_build_message = serde_json::from_str::<types::ApplicationBuildMessage>(message.body.unwrap().as_str()).unwrap();
            let application_build_context = serde_json::from_str::<types::ApplicationBuildContext>(&application_build_message.message).unwrap();
            let application_builder = ApplicationBuilder::new(application_build_context, application_build_message.message_attributes.authorization.value);
            ...
            match application_build_result {
                Ok(_) => {
                    ...
                    let delete_message_output = client.delete_message().queue_url(queue_url.clone()).receipt_handle(message.receipt_handle.unwrap()).send().await.unwrap();
